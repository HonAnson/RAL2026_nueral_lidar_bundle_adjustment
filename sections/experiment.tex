...
\begin{table*}
	...
	\begin{tabular}{l@{\hspace{2pt}}*{10}{c@{\hspace{7pt}}}}
		...
		NeLD-BA (Ours) &
			...
			\textbf{\rev{\res{rawcd}{ours}{mathhard}}} &      % mathhard
			\textbf{\res{rawcd}{ours}{quadeasy}} &      % quadeasy
			\textbf{\res{rawcd}{ours}{quadmid}} &      % quadmid
			\textbf{\rev{\res{rawcd}{ours}{quadhard}}} &      % quadhard
			\textbf{\rev{\res{rawcd}{ours}{canteenday}}} &      % canteenday
...
In this section, experiments were conducted to compare our method against state-of-the-art algorithms on mapping and trajectory performance. \rev{An ablation study is then presented}.
...
Our method \rev{produces} a more accurate pose refinement, evident from the substantial reduction in the L1-Chamfer distance of the raw maps \rev{from} our method compared to those \rev{from} HBA and BALM, as shown in Table~\ref{tab:pcd_plotting_cd-eval}, \rev{particularly} on challenging sequences such as quad\_hard and math\_hard. In contrast, HBA often fails to register point clouds effectively, while BALM \rev{fails} to handle outlier frames, which is also evident in Figure~\ref{fig:error_histogram}. Interestingly, raw maps generated using ground truth poses \rev{yield} relatively higher L1-Chamfer distances for several Newer College sequences, suggesting that the provided ground truth poses are not perfectly accurate and may introduce additional misalignments as reported in~\cite{zhong20244dNDF}. Qualitative results are shown in Figure~\ref{fig:raw_map_results}. Across sequences with both favorable initializations (e.g., quad\_easy) and challenging ones with poor initial poses (e.g., quad\_hard), our method consistently produces more coherent and structurally faithful raw maps compared to HBA and BALM.


\paragraph{Rendered Map Evaluation}
\rev{Benefiting} from the pose refinement capabilities, our method is able to produce a more accurate rendered map from NeRF against state-of-the-art mapping algorithms, which is particularly evident in the quad\_hard sequence in Figure~\ref{fig:rendered_map}. Table~\ref{tab:rendered_map} shows the \rev{quantitative} results for rendered maps. Our algorithm outperforms SHINE-Mapping, PIN-SLAM and 4dNDF on F1 score and L1-CD, while achieving \rev{comparable} inlier RMSE. For sequences with better initial poses such as quad\_easy, our rendering method is able to produce a cleaner map which reduces outliers\rev{, while others over-reconstruct the map,} which lowers their F1 score \rev{and} is also evident in the \rev{reduction} of high-error points shown in Figure~\ref{fig:rendered_map}.
...
\begin{table}
	...
	\end{tabular}
	{\captionsetup{justification=raggedright,singlelinecheck=false}
 	\caption*{\small The best results are highlighted in \textbf{bold}\rev{,} the second-best results are \underline{underlined}.}}
	\vspace{-20pt}
\end{table}

We also evaluate the optimized pose with trajectory ATE, and show that our method \rev{has substantially improved} the initial poses from FAST-LIO2. The quantitative and qualitative results are shown in Table~\ref{tab:ate-eval} and Figure~\ref{fig:trajectory_result}. Both deep learning approaches have underperformed as the pre-trained models struggle to generalize to outdoor scenes. \rev{In contrast,} BALM and our method \rev{have} achieved a near ground truth trajectory for FusionPortable sequences. Due to the potentially imperfect ground truth pose for Newer College sequences as discussed in Section~\ref{sec:pcd_plot_map}, trajectory evaluation for Newer College sequences is not provided.
...
\begin{table}
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		\makecell[l]{w/o \rev{Fine} Sampling \\ w/o Cube Bound} & 
		...
		\makecell[l]{Ours w/ \rev{Centered Fine} \\ Sampling~\cite{mildenhall2020nerfOG}} & 
		...
		\makecell[l]{\rev{Ours w/ Hash Grid}} & 
			\rev{\res{rawcd}{ablhash}{mathhard}} &      % mathhard
			\rev{\res{rawcd}{ablhash}{quadhard}} &      % quadhard
			\rev{\res{rawcd}{ablhash}{canteenday}} \\      % canteenday
		NeLD-BA (Ours) & 
			\textbf{\res{rawcd}{ablours}{mathhard}} &      % mathhard
			\textbf{\res{rawcd}{ablours}{quadhard}} &      % quadhard
			\textbf{\res{rawcd}{ablours}{canteenday}} \\      % canteenday
		\bottomrule
	\end{tabular}
	{\captionsetup{justification=raggedright,singlelinecheck=false}
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{\textbf{Ablation Study Qualitative Results \rev{on Raw Maps}.} \rev{Centered fine sampling}~\cite{mildenhall2020nerfOG} \rev{results} in an outlier frame (left)\rev{,} while our \rev{pre-termination fine sampling} successfully registered the outlier frame (right).}
  \label{fig:ablation_qualitative_result}
  \vspace{-20pt}
\end{figure}
\subsection{Ablation Study}
\rev{An ablation} study was conducted across three sequences to validate our algorithm. We quantitatively compare the ablation results by evaluating the raw point cloud map \rev{Chamfer} distance, as shown in Table~\ref{tab:ablation_study}. When the volume samples are not bounded within the $[-1,1]^3$ cube and \rev{fine} sampling is not used, the pose refinement performance significantly drops. It is also evident that the surrogate gradient and termination loss are crucial for achieving high-quality BA. Furthermore, our \rev{pre-termination fine sampling} is able to eliminate outlier frames in registration compared to NeRF's \rev{centered fine sampling}, evident in Figure~\ref{fig:ablation_qualitative_result}. \rev{Interestingly, using a hashgrid encoding also performed comparable with our method, which is a promising direction for future investigation.}

\subsection{Limitations}
\rev{Large-scale} sequences might be challenging \rev{for our method} due to the need to normalize point clouds within a $[-1,1]^3$ cube. Furthermore, our algorithm faces \rev{a challenge common to} NeRFs that it requires relatively long training times.