...
Table~\ref{tab:rendered_map} shows the qualitative results for rendered maps. Our algorithm outperforms SHINE-Mapping, PIN-SLAM and 4dNDF on F1 score and L1-CD, while achieving cmoparable inlier RMSE. Outperformance on hard sequences (e.g. quad\_hard) is preliminary due to the pose refinement capabilities of our algorithm. For easier sequences, our rendering method is able to produce a cleaner map which reduces outliers while others over reconstructs which lowers their F1 score. Qualitative results for mapping are shown in Figure~\ref{fig:rendered_map}, our method produces sharper maps while others produces map with more extra artifacts.
