...
\begin{figure*}[h!]
	...
	\caption{\textbf{Comparison of RGB NeRF-BA and LiDAR NeRF-BA in Volume Sampling along Depth/Range.}
		...
		(b) Sampling around the sharp transition of $\sigma(z)$ (highlighted pink region) is critical to produce a difference in the ray termination distribution $h(z)$ and $h(z+\epsilon)$ after alpha blending, which provides non-vanishing $\frac{\partial \hat{h}(z_i)}{\partial \mathbf{x}_i}$ for pose optimization.
...
\begin{figure}
	...
	\caption{\textbf{Ray Termination Distributions of a LiDAR Ray in canteen\_day sequence} Our sampling method better approximates the baseline shifted distribution ($\epsilon=0.01$) constructed with 8192 uniform samples, which is critical for providing accurate gradients in LiDAR NeRF-BA.}
