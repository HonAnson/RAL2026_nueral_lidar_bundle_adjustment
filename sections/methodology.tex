...
\begin{figure}
	...
	\caption{\textbf{Ray Termination Distributions of a LiDAR Ray in canteen\_day sequence} We illustrate the idea in Figure~\ref{fig:rgb_nerf_ba_vs_lidar_nerf_ba}(b) with an actual LiDAR ray. Our sampling method better approximates the baseline distribution constructed with 8192 uniform samples when shifted ($\epsilon=0.01$), which is critical for providing accurate gradients in LiDAR NeRF-BA.}
