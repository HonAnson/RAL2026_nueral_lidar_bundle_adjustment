...
To address these challenges, we show that LiDAR NeRF-BA performance is tightly coupled with volume-sampling density, and propose a volume-sampling strategy tailored to the LiDAR ray model. Given a noisy initialization of LiDAR poses over a sequence, our algorithm, termed NeLD-BA, jointly reconstructs the scene and refines the poses, outperforming existing LiDAR NeRF methods in both novel view synthesis and odometry (Figure~\ref{fig:intro_example}).

Our main contributions in this work include: 1) We provide new insights that pose accuracy in LiDAR NeRF-BA is explicitly reinforced by volume-sampling positions; 2) We propose simple yet efficient strategies for volume sampling and positional encoding tailored to LiDAR NeRF-BA, together with a LiDAR-specific loss function; 3) Extensive experiments against state-of-the-art algorithms demonstrate superior mapping and localization performance. 
